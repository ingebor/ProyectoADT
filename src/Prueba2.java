...
public class Prueba2 {
	...
	public void Cons (LObj a , LObj d) {
		car = a;
		cdr= d;
	}
	
	public LObj callSubr(LObj args) {
		return args;
	}
	
	public void Expr(LObj a, LObj b, LObj e) {
		args = a;
		body = b;
		env = e;
	}
    ...
    public void LObj(Type type, Object obj) {
        tag_ = type;
        data_ = obj;
    }
    public Integer num() {
    ...
    private String listToString(LObj prueba2) {
        String ret = "";
        boolean first = true;
        while (prueba2.tag() == Type.CONS) {
            if (first) {
                first = false;
            } else {
                ret += " ";
            }
            ret += prueba2.cons().car.toString();
            prueba2 = prueba2.cons().cdr;
        }
        if (prueba2.tag() == Type.NIL) {
            return "(" + ret + ")";
        }
        return "(" + ret + " . " + prueba2.toString() + ")";
    }
    ...
    public void ParseState (LObj o, String s) {
		obj = o;
		next = s;
	}
